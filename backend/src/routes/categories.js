...
router.delete('/:categoryId', verifyToken, requireRole(['SUPER_ADMIN', 'ADMIN']), (req, res) => {
  try {
    const { categoryId } = req.params;
    ...
    if (docCount.count > 0) {
      return res.status(400).json({ error: 'Cannot delete category with active documents' });
    }
