...
function generateDocumentId() {
  const lastDoc = db.prepare(
    'SELECT document_id FROM documents ORDER BY id DESC LIMIT 1'
  ).get();
...
router.post('/upload', verifyToken, requireRole(['SUPER_ADMIN', 'ADMIN']), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
...
router.get('/', verifyToken, (req, res) => {
  try {
    ...
    const countQuery = 'SELECT COUNT(*) as total FROM documents WHERE status = "ACTIVE"' +
...
router.get('/:documentId/qr', verifyToken, (req, res) => {
  try {
    ...
    const qr = db.prepare(
      'SELECT qr_code_path FROM qr_codes WHERE document_id = ?'
    ).get(documentId);
...
router.delete('/:documentId', verifyToken, requireRole(['SUPER_ADMIN', 'ADMIN']), (req, res) => {
  try {
    const { documentId } = req.params;
